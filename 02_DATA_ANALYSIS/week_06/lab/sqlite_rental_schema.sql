...
Rental for SQLite is a port of the Rental example database available for MySQL, which was originally developed by Mike Hillyer of the MySQL AB documentation team. 
...
GROUP_CONCAT(DISTINCT CONCAT(c.name, ': ',
        (SELECT GROUP_CONCAT(f.title ORDER BY f.title SEPARATOR ', ')
                    FROM Rental.film f
                    INNER JOIN Rental.film_category fc
                      ON f.film_id = fc.film_id
                    INNER JOIN Rental.film_actor fa
...
AS film_info
FROM Rental.actor a
LEFT JOIN Rental.film_actor fa
  ON a.actor_id = fa.actor_id
LEFT JOIN Rental.film_category fc
  ON fa.film_id = fc.film_id
LEFT JOIN Rental.category c
